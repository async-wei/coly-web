...
class USNCOParser:
    ...
    def _extract_answer_key(self, page):
        """answer key from last page"""
        text = page.extract_text()

        # answer key is in two-column format, pattern to find pairs
        pattern = r'(\d+)\.\s+([A-D])'
    ...
    def _remove_footer_text(self, text: str, page_number: int) -> str:
        """
        Remove footer text from pages.
        Odd pages (3, 5, 7): "Property of ACS USNCO – Not for use as USNCO Local Sectio"
        Even pages (4, 6): "Page [number] Property of ACS USNCO –"
        """
    ...
    def _clean_scientific_notation(self, text: str) -> str:
        ...
        unicode character \uf0b4 for multiplication.
        """
        text = text.replace('\uf0b4', '×')
        # eg. 10XX where XX is the exponent
    ...
    def _parse_question_page(self, page, page_number: int):
        """parse q with two column format"""
        width = page.width
        height = page.height
        ...
        # Check for images in each column (not working)
    ...
    def _parse_column_text(self, text: str, page_number: int, has_images: bool = False):
        """Parse questions from a column of text"""
        # scientific notation and subscripts
    ...
    def _merge_subscript_lines(self, text: str) -> str:
        """
        Merge lines where subscripts appear on separate lines.
        algorithm to handle multiple formulas and nested parentheses.
        """
        lines = text.split('\n')
        merged_lines = []
        i = 0
        ...
        while i < len(lines):
            ...
            # Check if next line is likely a subscript line
            if i + 1 < len(lines):
                next_line = lines[i + 1].strip()

                if self._is_subscript_line(next_line, current_line):
                    # Merge next line as subscripts
    ...
    def _is_subscript_line(self, line: str, prev_line: str) -> bool:
        """Determine if a line is likely a subscript line."""
        if not line:
            return False

        # Line with only digits and spaces
        if re.match(r'^[\d\s]+$', line):
            if prev_line and re.search(r'[A-Za-z\)\]]$', prev_line):
                # Subscript line should be short (< 30 chars)
                if len(line) < 30:
                    return True
    ...
    def _merge_with_subscripts_advanced(self, line: str, subscript_line: str) -> str:
        ...
        # Find insertion points for subscripts
        insertion_points = self._find_subscript_insertion_points(line)

        # Match tokens to insertion points
        if len(tokens) == len(insertion_points):
            ...
            for pos, token in zip(insertion_points, tokens):
                ...
                # Check if we're replacing a space or inserting
                if insert_pos < len(result) and result[insert_pos] == ' ':
                    # Replace space with subscript
    ...
    def _find_subscript_insertion_points(self, line: str) -> List[int]:
        """
        Find positions where subscripts should be inserted.
        Uses context-aware matching for chemical formulas.
        """
        insertion_points = []
        all_matches = []

        #Chemical formulas followed by space or delimiter
        for match in re.finditer(r'([A-Z][a-z]?)\s', line):
            pos_before = match.start()
            element = match.group(1)

            # Skip if this is an answer choice marker like "(A) " or Roman numeral
            if pos_before > 0 and line[pos_before - 1] == '(':
                # Check if it's a single letter (likely answer choice)
                if len(element) == 1:
                    continue

            all_matches.append((match.start(), match.end() - 1, element))

        # Filter adjacent elements eg. H O keep only H to handle cases like H2O
        filtered_matches = []
        for i, (start, end, element) in enumerate(all_matches):
            # Check if next match is immediately adjacent
            if i + 1 < len(all_matches):
                next_start, _, _ = all_matches[i + 1]
                # Keep only the FIRST element (current one)
                if next_start - end <= 3:
                    # This and next are adjacent - keep current, skip next
                    filtered_matches.append(end)
                    continue
            if i > 0:
                prev_start, prev_end, _ = all_matches[i - 1]
                if start - prev_end <= 3:
                    # Previous was kept, skip this one
                    continue
            filtered_matches.append(end)

        insertion_points.extend(filtered_matches)

        # Element inside brackets with space for stuff like kinetics for example [Br ]
        for match in re.finditer(r'\[([A-Z][a-z]?)\s*\]', line):
            insertion_points.append(match.end(1))

        # Closing parenthesis (for nested formulas like (SO4)2
        for match in re.finditer(r'\([^)]*[A-Z][a-z]?\s*\)', line):
            paren_content = match.group()
            # Check if content is just a single letter (answer choice/Roman numeral)
    ...
    def _process_question_block(self, block: str, q_number: int, page_number: int, has_images: bool):
        """Process a single question block to extract question text and choices"""
        # Remove question number from start
        block = re.sub(r'^\s*\d+\.\s+', '', block, count=1)

        # Extract choices and convert inline exponents to superscripts
        choices = {}
        question_lines = []
        choice_lines = []
        ...
        lines = block.split('\n')
        for line in lines:
            # Check if line contains answer choices
    ...
    def _convert_rate_law_exponents(self, text: str) -> str:
        """
        Convert inline numbers after brackets to superscripts (rate law exponents), 
        numbers that appear IMMEDIATELY after ]
        """
        superscript_map = {
            '0': '⁰', '1': '¹', '2': '²', '3': '³', '4': '⁴',
            '5': '⁵', '6': '⁶', '7': '⁷', '8': '⁸', '9': '⁹'
        }
        # Pattern: ]<digit> where digit is the exponent
        # Match ][digit] where digit is 1-9
        pattern = r'\]([1-9])'
    ...
    def _calculate_stats(self) -> Dict:
        ...
        high_conf = sum(1 for q in self.questions if q.parsing_confidence == 'high')
        medium_conf = sum(1 for q in self.questions if q.parsing_confidence == 'medium')
        low_conf = sum(1 for q in self.questions if q.parsing_confidence == 'low')
        with_answers = sum(1 for q in self.questions if q.correct_answer)
        with_4_choices = sum(1 for q in self.questions if len(q.choices) == 4)
        return {
            ...
            "questions_with_4_choices": with_4_choices,
            "success_rate": f"{(with_4_choices / total * 100):.1f}%" if total > 0 else "0%"
        }
